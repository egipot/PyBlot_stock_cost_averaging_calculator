...
st.markdown(''':orange[4. Warning: Switching the page from the sidebar will restart the app, which means you need to select the function [add, view, edit, calculate, remove].]''')
st.markdown(''':red-background[5. The formula of calculations are based on the assumption that **broker fees are applied in both buying and selling transactions.** And that **tax is applied only when stocks are sold.**]''')
